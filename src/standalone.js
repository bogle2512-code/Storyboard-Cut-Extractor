...
function exportSizeForCut(cut, sizeKey) {
  const box = selectedBox(cut);
...
async function exportCutBlob(cut, sizeKey = 'original') {
  ...
  const { size } = exportSizeForCut(cut, sizeKey);
  const canvas = document.createElement('canvas');
...
function exportLabelForCut(cut) {
  ...
  return `${first.width} x ${first.height}${extra}, ${exportButtonLabel()}, 꽉 차게`;
}
...
async function generateExportPreview() {
  ...
  for (const [cutIndex, cut] of state.cuts.entries()) {
    ...
    for (const sizeKey of state.exportSizes) {
      ...
      const size = exportSizeForCut(cut, sizeKey).size;
...
async function downloadZip() {
  ...
  const metadata = {
    ...
    exportOptions: {
      sizes,
      exportMode: 'selected-image-cover-resize',
...
document.querySelectorAll('input[name="resizeMode"]').forEach((input) => {
  input.addEventListener('change', () => {
    state.resizeMode = 'cover';
    renderCuts();
    setStatus('선택한 이미지를 각 출력 사이즈에 꽉 차게 저장합니다.', false);
  });
});
